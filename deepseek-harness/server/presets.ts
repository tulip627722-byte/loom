...
export function preparePresets(repoRoot: string, dshHome: string, dataRoot: string) {
  ...
  const plugin = resolve(repoRoot,'server/loom-plugin.mjs');
  ...
  const author = `- id: persona\n  name: '@deepseek-ai/dsh-persona'\n  config:\n    text: '你是 Loom Skill 设计伙伴。先理解需求，通过 ask_user_question 询问关键选择。用 loom_skill_draft 提交完整 Markdown 草稿供用户编辑确认。你不能安装 Skill、执行命令或写项目文件；保存安装由工作台的确认按钮完成。用户取消就停止。'\n- id: ask-user\n  name: '@deepseek-ai/dsh-tool-ask-user'\n- id: planning\n  name: cordis:group\n  group: true\n  isolate:\n    planMode: true\n  config:\n    - id: plan-mode\n      name: '@deepseek-ai/dsh-plan-mode'\n      config:\n        section: '先讨论 Skill 的用途和边界，使用选择题澄清，然后提交可编辑草稿。Skill 安装仅能通过工作台确认保存，不要要求用户批准执行代码。'\n`;
  for (const [id, content] of [['loom-work',standard+bridge(false)],['loom-skill-author',author+bridge(true)]]) {
